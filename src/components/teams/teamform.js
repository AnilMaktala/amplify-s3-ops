...
function TeamForm({ setShowForm, trigger }) {
    ...
    return (
            ...
            onSubmit={(event) => {
                ...
                try {
                    ...
                    const response = API.graphql(
                        graphqlOperation(createTeam, { input: state })
                    );
                    setShowForm(false);
                    trigger();
                } catch {
        ...
        >
            ...
            >
                <Container header={<Header variant="h2">Add Team</Header>}>
                    <SpaceBetween direction="vertical" size="l">
                        ...
                        <FormField
                            label="Organization"
                            secondaryControl={
                                <Button onClick="loadOrgs" iconName="refresh" />
                            }
                        >
                            <Select
                                selectedOption={selectedOrg}
                                onChange={({ detail }) => setSelectedOrg(detail.selectedOption)}
