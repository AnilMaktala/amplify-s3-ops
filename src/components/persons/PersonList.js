...
import { fetchActivePlan, fetchPersons, removePerson } from '../../common/graphqlHelper'
...
function PersonList() {
    ...
    const [preferences, setPreferences] = useState({
        pageSize: 50,
        visibleContent: ["alias", "name", "email", "status", "sixManagerAlias", "managerAlias"],
    });
    const {
        items,
        actions,
        ...
        filtering: {
            empty: (
                <EmptyState title="No Persons" action={<Button>Create Person</Button>} />
            ),
            noMatch: (
                <EmptyState
                    title="No matches"
    ...
    const load = async (planId) => {
        let orgId = (activeIsb || overActiveIsb) ? null : Cache.getItem("activeOrg").id
        let token = Cache.getItem("nextToken");

        setLoading(true);
        var items = []
        do {
            const res = await fetchPersons(planId, orgId, token);
            items = [...items, ...res]
    ...
    function handleDelete() {
        if (selectedItems.length === 1) {
            ...
            if (id) removePerson(id).then((res) => { console.log(res); })
        }
    }
                ...
                {showForm && (
                    <Container>
                        <SpaceBetween direction="vertical" size="l">
                            <PersonForm setShowForm={setShowForm} trigger={trigger}
                                plan={activePlan}
                                org={activeOrg}
